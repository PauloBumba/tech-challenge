...
|| TASK-FE-01 | Frontend — listagem de Beneficiários: módulo beneficiarios/, filtros combináveis, paginação, nome do plano via GET /planos | 1363b2e | build Angular GREEN |
|| TASK-FE-02 | Frontend — formulário cadastro/edição: reactive forms, CPF espelhando CpfValidator, CPF não editável na edição | 1363b2e | build Angular GREEN |
...
|| TASK-OBS-01 | Observabilidade: /metrics em Prometheus (System.Diagnostics.Metrics nativo, contagem+duração por rota/status) + log estruturado de requisição (SPEC §7); teste de integração RED→GREEN | (a preencher) | 84/84 |
|| TASK-OBS-02 | Corrigir sintaxe Prometheus dos buckets de duração: label `le` dentro das chaves da série; teste RED→GREEN | (a preencher) | 85/85 |
|| TASK-FE-07 | Frontend — indicador de saúde da API no topo (GET /health, a cada 30s; online/indisponível); 2 testes E2E | 6888db7 | npm run e2e GREEN (3 passed) |
|| TASK-ENT-01 | Build multi-arch e publicação no Docker Hub: bumab/desafio-4tech-api:latest e bumab/desafio-4tech-web:latest (linux/amd64,linux/arm64) | — | verificar.sh 37/37 ok |
|| TASK-ENT-02 | Estrutura de entrega: participantes/paulobumba/ com docker-compose.yml, info.json, README.md completo | — | verificar.sh 37/37 ok |
|| TASK-ENT-03 | Verificação: ./verificar.sh participantes/paulobumba → 37 ok, 0 falha(s) de 37 verificações | — | verificar.sh 37/37 ok |
|| TASK-ENT-04 | README de entrega: Resumo, Decisões, Uso de IA, Perguntas compilados a partir de DECISIONS.md e AI_USAGE.md | — | verificar.sh 37/37 ok |
