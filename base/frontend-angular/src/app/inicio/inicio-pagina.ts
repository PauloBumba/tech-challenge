  ...
  template: `
    <section class="space-y-6">
      ...
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <app-card-metrica titulo="Beneficiários" [valor]="totalBeneficiarios()" descricao="Total retornado pela listagem" />
        <app-card-metrica titulo="Planos ativos" [valor]="totalPlanos()" descricao="Planos disponíveis na API" />
        <app-card-metrica titulo="Requisições observadas" [valor]="totalRequisicoes()" descricao="Contador atual da API" />
        <app-card-metrica titulo="Banco de dados" [valor]="banco()" descricao="Resultado do health check" />
      </div>
...
export class InicioPagina {
  ...
  constructor() {
    ...
    this.health.obter().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: r => this.banco.set(r.banco === 'ok' ? 'Online' : r.banco), error: () => this.banco.set('Indisponível') });
  }
